...
decision_link: /ops/decisions/dec-2025-11-draft-move
related_decision: /decisions/dec-2025-11-ia-overhaul.md
date: '2025-11-15'
---

# Northbook Operations Contract v1.0

Keep the work predictable and receipts visible. [Apply this contract](#purpose) or [Review the release context](../../docs/release.md).

State: [State visibility map](../../docs/operate/state-visibility.md) · Ledger: [Release state](../../docs/navigate/state-ledger.md) · Release reference: [site-v2025.11 bundle](../../ops/releases/2025-11/index.md)
...
## Draft move protocol (decision dec-2025-11-draft-move)

The draft-move decision is final. When a page meets the irrelevant criteria, the owner must quarantine it under `/draft`, keep traceability intact, and register the move on the next release tag.

### Classification (v0.1)

- Out-of-scope topic (no live bucket, guardrail, or contract mapping).
- Stale owner/date or missing refresh commitment.
- Unrunnable guidance (cannot be executed in 10 minutes or lacks a measurable CTA path).
- Duplicative of a canonical source already tagged for pilots.

### Move checklist

1. Keep `owner`, `date`, `bucket_id`, `page_id` in frontmatter and add `decision_id: dec-2025-11-draft-move`, `move_reason`, and a `receipts` pointer to the release entry.
2. Update the matching `/ops` record with the new `/draft` location, owner, and timestamp.
3. Ensure docs:guard, page-scan, and VitePress ignore rules already cover the new path—no new CI hooks allowed in v0.1.
4. Run page-scan to confirm zero broken links and at most one redirect hop from the previous URL.

### Receipts and escalation

- Release tags (starting with `site-v2025.11`) must list every move with the prior path, `/draft` path, owner, and `move_reason`.
- Metrics: `m-search-noise` should improve within one tag; `m-link-warnings` stays at zero; `m-draft-restore-rate` <= 10% over two cycles.
- Freeze new moves if any receipt is missing, broken links surface, or search noise worsens for 7 consecutive days. Resume only after metrics return within 10% of baseline and receipts are complete.
